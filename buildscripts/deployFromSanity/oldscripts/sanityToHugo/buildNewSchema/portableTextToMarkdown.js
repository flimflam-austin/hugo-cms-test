...
const serializers = {
    types: {
        image: node => {
            ...
            if (!asset) {
                throw Error(`Could not extract image ref from node: ${JSON.stringify(node)}.`)
            }
            ...
            if (!imageUrl) {
                throw Error(`Failed to extract image url from node: ${JSON.stringify(node)}`)
            }
...
const portableTextToMarkdown = portableText => {
    if (!portableText) {
        throw Error('Could not parse portable text, value is falsey.');
    }
    ...
    try {
        ...
        throw Error(`Error converting portable text at portableTextToMarkdown in portableTextToMarkdown. \nError: ${err.message}\n`)
    }
};
